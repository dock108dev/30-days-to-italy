...
> Historical design and implementation record preserved during documentation cleanup. Use the [current teaching contract](../DAY_00_01_TEACHING_SPEC.md) for behavior. Several original external evidence files are unavailable; their retained references below are historical locations, not accessible current proof.
...
Sections 2–6 describe the implementation contract; T1 baseline inspection and its handoffs below remain design history. Engineering verification is not comprehension or owner acceptance. Overall **REVISE DEMO — NOT READY FOR ANOTHER DEMO; D0-01 OPEN**. The [active tracker](/Users/michaelfuscoletti/Desktop/italy_next_steps.md) owns current work status. The tracker records historical T1–T5 completion for `d52fd1f` and an unrun T6 owner review. The September 23 audit found the external `italian-pilot-evidence` directory unavailable, so linked historical results are not freshly verified. The expanded beta now starts with B1 full-season teaching/review/variation planning; newer source requires applicable qualification.
