...
Optional `EpisodeDefinition.preparation` owns all teaching content under Day 0/1 and obtains transcript/media from owned registry turns. `prototype/preparation.ts` derives the outer gate from authoritative episode, turn, status, content version and traversal. Its exact cursor includes segment, mode, page and navigation revision; old and duplicate actions cannot move a newer page. The composer submission coordinator checks this gate as well as session, turn and submission-in-flight guards. No engine evaluator changes or alternative assessment authority exist.

Preparation transitions update only optional current-attempt `GameState.preparation`. They never call support, response evaluation, phrase practice, result construction or deck handoff. Entry example traversal is recorded on explicit pattern Continue; segment traversal on Start conversation. Later briefs record both on Continue conversation. Review mode persists return turn/status/page; return revalidates authoritative state. Live phase, draft and focus remain presentation-owned. Explicit replay clears preparation through the existing interaction reset.

`game/persistence.ts` normalizes additive schema-6 preparation without changing the game storage key. Invalid or mismatched active content resets preparation alone; terminal/resolved precedence preserves world state. Historical result summaries use a bounded independent parser that retains recorded versions/IDs without requiring current titles. `buildObservedEpisodeResult` copies only version, unique traversal IDs and per-segment audio counts in the existing actual-result transaction; no cursor or response text is added to that summary. Eight-result and attempt identity rules remain unchanged.

`PreparationView` owns cancelable independent audio and records attempted starts, including failures, only for active attempts. Completed detour playback is transient. `usePrototypePresentation` owns heading/return focus and suppresses hidden-composer focus. Live audio promises are guarded against changed turns, pages and sessions. Read the line counts a first visible reveal through existing transcript support, independently of preparation reading.

The existing session hook remains the sole save authority. Demo namespacing and owner restoration include preparation in the game domain; no new domain or storage key exists. Integrated checks cover ordinary UI traversal, synthetic Admin restoration and all checkpoint paths separately. Preparation and offline acceptance inspect service-worker control and preserved cache inventory; they do not change any state authority. Focused or integrated engineering checks do not establish owner acceptance.
