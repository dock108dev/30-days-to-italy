...
export const day03Episode: EpisodeDefinition = {
  ...metadata, sceneId: "morning-bar",
  scene: { id: "morning-bar", episodeId: "day-03", day: "Day 3", dateLabel: "28 days out", title: metadata.title, location: metadata.location, time: "08:35", npc: "Giulia", role: "Bartender", objective: "Order one €2 espresso, here or takeaway.", firstTurn: "d03_01_order", kicker: "This is your first visit; Giulia does not know your usual yet.", suggestions: ["Vorrei un espresso.", "Qui, grazie. / Da portare, grazie.", "Con la carta."] },
  turns: {
  ...
  outcomes: {
    "D03-O1": { id: "D03-O1", title: "Espresso at the bar", detail: "You ordered one espresso, chose to drink it here, and paid exactly €2.00.", consequence: "Drink here · −€2.00 · Giulia met", tone: "success" },
    ...
    "D03-O3": { id: "D03-O3", title: "Espresso to go", detail: "You ordered one takeaway espresso and paid exactly €2.00.", consequence: "Takeaway · −€2.00 · Giulia met", tone: "success" },
  },
  terminalOutcomeTurns: { "D03-O1": ["d03_04_close"], "D03-O3": ["d03_04_close"] },
  evaluateResponse({ state, normalized, createId, runtime }) {
    ...
    if (state.turnId === "d03_02_here") {
      ...
      if (any(normalized, ["da portare", "to go", "takeaway", "take away", "porto via"])) return runtime.moveToTurn(state, "d03_03_pay", {}, undefined, createId);
      if (exit) return runtime.resolveOutcome(state, "D03-O2", {}, null, createId);
      return runtime.moveToTurn(state, "d03_02_here", {}, "Say qui to stay or da portare for takeaway.", createId);
    }
    if (state.turnId === "d03_03_pay" && (any(normalized, PAY) || anyWholePhrase(normalized, YES))) return runtime.queueTerminal(state, "d03_04_close", state.verifiedFacts.preferenceSelected === "takeaway" ? "D03-O3" : "D03-O1", {
  ...
  observeResponse({ before, after, normalized }) {
    ...
    if (before.turnId === "d03_02_here" && after.turnId === "d03_03_pay") {
      const takeaway = any(normalized, ["da portare", "to go", "takeaway", "take away", "porto via"]);
      return observation(["preference"], { preferenceSelected: takeaway ? "takeaway" : "drink here" });
    }
